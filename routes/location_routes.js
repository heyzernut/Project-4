...
router.post('/new', (req, res) => {
  ...
  newLocation.save().then(() => res.redirect('/location'), err => res.send(err))
})
...
router.put('/:id', (req, res) => {
  ...
  if (formData.zone_start) {
    ...
    }).then(() => res.redirect(`/location`)).catch(err => console.log(err))
  } else {
    Location.findByIdAndUpdate(req.params.id, {address: formData.address}).then(() => res.redirect(`/location`)).catch(err => console.log(err))
  }
})

router.delete('/:id', (req, res) => {
  Location.findByIdAndRemove(req.params.id).then(() => res.redirect('/location')).catch(err => console.log(err))
})
